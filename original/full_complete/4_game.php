<?php
    ...
    if ($mode === "easy") {
        $stmt = $dbh->query("SELECT id, name, image FROM beans ORDER BY RAND()");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $beans = array_map(function ($row) {
            return [
                "bean_id" => $row["id"],
                "name" => $row["name"],
                "image" => $row["image"] ?? "",
        ...
        $stmt = $dbh->query("SELECT id, name, info, ruby, image FROM beans ORDER BY RAND()");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $beans = [];
...
<html lang="ja">
    ...
    <body><!--表示内容-->
        ...
        <script>//処理内容
           ...
           const IS_CHALLENGE = <?= $challenge ? 'true' : 'false' ?>;
           //モード選択用
...
//関数定義ゾーン
            ...
            function render() {
                ...
                if (IS_CHALLENGE) {
                    const challengeText = currentType === "feature" ? "特徴を入力してください" : "名前を入力してください";
                    html += `<div class="challenge-label">画像を見て内容を入力してください</div>`;
                    html += `<div class="type-label">${challengeText}</div>`;
                    if (currentImage) {
                        html += `<img class="challenge-image" src="${currentImage}" alt="問題画像">`;
                    }
                    if (typed) {
                        html += `<div class="typed-feedback">${typed}</div>`;
                    }
                } else {
                    if (currentType === "feature") {
                        html += `<div class="type-label">特徴を入力してください</div>`;
                    } else if (currentType === "name") {
                        html += `<div class="type-label">名前を入力してください</div>`;
                    }
            ...
            function nextWord() {
                ...
                currentImage = beans[currentIndex].image ?? "";
                currentType = beans[currentIndex].type ?? "name";
                currentBeanId = beans[currentIndex].bean_id ?? null;
                // 現在の単語（日本語）を取得
                const currentRuby = beans[currentIndex].ruby ?? currentWord;
                
                // チャレンジモード + 特徴後 の場合、同じ㋂IDの全特徴を候補に一覧化
                if (IS_CHALLENGE && currentType === "feature" && currentBeanId !== null) {
                    candidates = [];
                    beans.forEach(function(bean) {
                        if (bean.bean_id === currentBeanId && bean.type === "feature") {
                            var ruby = bean.ruby ?? bean.name;
                            var romajis = getRomajiCandidates(ruby);
                            candidates = candidates.concat(romajis);
                        }
                    });
                    candidates = [...new Set(candidates)]; // 重複を除去
                } else {
